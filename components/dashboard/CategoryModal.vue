<template>
  <UiModal :isOpen="isOpen" :title="category ? 'Edit Category' : 'New Category'" description="Organize your menu with beautiful categories" size="sm" @close="$emit('close')">
    ...
    <template #footer>
      <button @click="$emit('close')" class="px-6 py-2 text-xs font-bold text-gray-400 uppercase tracking-widest hover:text-gray-900 transition-colors">Cancel</button>
      <button @click="handleSubmit" :disabled="loading" class="px-6 py-2.5 bg-[#065fdb] text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:brightness-110 active:scale-95 transition-all flex items-center gap-2 shadow-lg shadow-[#065fdb]/20">
