    ...
    <div v-if="isOpen" class="fixed inset-0 z-[200] flex items-center justify-center p-4">
      ...
      <div class="relative bg-white rounded-[2.5rem] shadow-2xl max-w-md w-full overflow-hidden transform transition-all group border border-white">
        ...
        <div class="p-8 bg-gray-50/50 flex flex-col sm:flex-row gap-4 border-t border-gray-100">
          <button 
            type="button" 
            class="flex-1 py-4 px-6 bg-white border border-gray-100 text-gray-400 text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-gray-100 hover:text-gray-900 transition-all active:scale-95 shadow-sm"
            @click="$emit('cancel')"
          >
            Go Back
          </button>
          <button 
            ...
            class="flex-1 py-4 px-6 text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-lg transition-all active:scale-95"
            @click="$emit('confirm')"
          >
            {{ confirmText || 'Proceed' }}
          </button>
