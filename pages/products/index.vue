<template>
 ...
 <span class="px-3 py-1 bg-gray-100 text-gray-600 rounded-md text-sm font-bold border border-gray-200/50">
 {{ selectedProduct.category }}
 </span>
 <span :class="selectedProduct.isAvailable ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-500 border-rose-100'" class="px-3 py-1 rounded-md text-sm font-bold border">
 ...
 <p class="text-sm font-bold text-gray-400 mb-2">Description & Notes</p>
 <p class="text-sm font-medium text-gray-700 leading-relaxed">{{ selectedProduct.description }}</p>
 </div>

 <div class="pt-4 border-t border-gray-100 flex flex-col gap-3">
 <h4 class="text-sm font-bold text-gray-400 mb-2">Item Actions</h4>
 
 <button 
 @click="handleToggleAvailability(selectedProduct)" 
 class="w-full px-5 py-2 bg-white text-gray-700 rounded-md text-[11px] font-bold border border-gray-200 hover:border-gray-300 active:scale-[0.98] transition-all flex items-center justify-between"
 >
 ...
 class="w-full px-5 py-2.5 bg-rose-50 text-rose-500 rounded-md text-[11px] font-bold border border-rose-100 hover:bg-rose-100 active:scale-[0.98] transition-all flex items-center justify-center gap-2 mt-2"
 >
 ...
 <h2 class="text-xl font-bold text-gray-900 tracking-tight font-display">List New Product</h2>
 ...
 <label class="text-sm font-bold text-gray-400 ml-1">Product Name</label>
 <input v-model="newProduct.name" placeholder="e.g. Authentic Suya Skewers" class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-md text-sm font-bold focus:ring-2 focus:ring-parentPrimary/20 focus:bg-white focus:border-parentPrimary/30 outline-none transition-all" required />
 </div>
 
 <div class="space-y-1.5">
 <label class="text-sm font-bold text-gray-400 ml-1">Description & Ingredients</label>
 <textarea v-model="newProduct.description" placeholder="Specify portion size, spice levels, or packaging details..." class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-md text-sm font-medium focus:ring-2 focus:ring-parentPrimary/20 focus:bg-white focus:border-parentPrimary/30 outline-none transition-all h-24 resize-none" />
 </div>
 ...
 <input v-model.number="newProduct.price" type="number" placeholder="0" class="w-full pl-8 pr-4 py-3 bg-gray-50 border border-gray-100 rounded-md text-sm font-medium focus:ring-2 focus:ring-parentPrimary/20 focus:bg-white focus:border-parentPrimary/30 outline-none transition-all" required />
 </div>
 </div>
 <div class="space-y-1.5">
 <label class="text-sm font-bold text-gray-400 ml-1">Category</label>
 <input v-model="newProduct.category" placeholder="snacks, mains, etc." class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-md text-sm font-bold focus:ring-2 focus:ring-parentPrimary/20 focus:bg-white focus:border-parentPrimary/30 outline-none transition-all r" required />
 </div>
 </div>

 <!-- Pre-order Toggle & Config -->
 <div class="pt-4 space-y-4">
 <div class="flex items-center justify-between p-4 bg-gray-50 rounded-md border border-gray-100 cursor-pointer hover:bg-gray-100/50 transition-all" @click="newProduct.isPreOrder = !newProduct.isPreOrder">
 <div>
 ...
 <div class="w-10 h-5 rounded-md relative transition-colors duration-300" :class="newProduct.isPreOrder ? 'bg-parentPrimary' : 'bg-gray-200'">
 <div class="absolute top-1 left-1 w-3 h-3 bg-white rounded-md transition-transform duration-300" :class="newProduct.isPreOrder ? 'translate-x-5' : ''" />
 </div>
 </div>
 ...
 <div class="grid grid-cols-2 gap-4">
 <div class="space-y-1.5">
 <label class="text-sm font-bold text-gray-400 ml-1">Order Deadline</label>
 <input v-model="newProduct.preOrderDeadline" type="datetime-local" class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-md text-sm font-bold focus:ring-2 focus:ring-parentPrimary/20 outline-none transition-all" />
 </div>
 <div class="space-y-1.5">
 <label class="text-sm font-bold text-gray-400 ml-1">Delivery Date</label>
 <input v-model="newProduct.availableDate" type="date" class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-md text-sm font-bold focus:ring-2 focus:ring-parentPrimary/20 outline-none transition-all" />
 </div>
 </div>
 <div class="space-y-1.5">
 <label class="text-sm font-bold text-gray-400 ml-1">Pre-order Note</label>
 <input v-model="newProduct.preOrderNote" placeholder="e.g. Freshly baked every Saturday morning" class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-md text-sm font-bold focus:ring-2 focus:ring-parentPrimary/20 outline-none transition-all" />
 </div>
 </div>
 </Transition>
 </div>
