  ...
  <div class="space-y-4 animate-fade-in max-w-7xl mx-auto pb-10">
    ...
    <div class="mb-6">
      <div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-6">
        ...
        <button @click="openAddDrawer" class="px-6 py-2.5 bg-parentPrimary text-white rounded-full font-bold text-xs shadow-sm shadow-parentPrimary/20 hover:brightness-110 active:scale-[0.98] transition-all uppercase tracking-widest flex items-center gap-2">
        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-50/50">
            <tr v-for="product in filteredProducts" :key="product._id" class="hover:bg-gray-50/80 transition-colors group cursor-pointer" @click="selectedProduct = product">
              ...
              <td class="py-4 px-2">
                <span class="px-2.5 py-1 bg-gray-100 text-gray-600 rounded-md text-[10px] font-bold uppercase tracking-widest border border-gray-200/50">{{ product.category }}</span>
              </td>
              <td class="py-4 px-2 text-right">
                <p class="text-[13px] font-bold text-gray-900 tracking-tighter">₦{{ product.price?.toLocaleString() }}</p>
              </td>
              <td class="py-4 px-2 text-right">
                <span :class="product.isAvailable ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-500 border-rose-100'" class="text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-md border inline-block">
      ...
      <template v-if="selectedProduct">
        <div class="flex flex-col items-center justify-center py-6 border-b border-gray-100">
           ...
           <div class="mt-4 flex gap-2">
             <span class="px-3 py-1 bg-gray-100 text-gray-600 rounded-full text-[10px] font-bold uppercase tracking-widest border border-gray-200/50">
               {{ selectedProduct.category }}
             </span>
             <span :class="selectedProduct.isAvailable ? 'bg-emerald-50 text-emerald-600 border-emerald-100' : 'bg-rose-50 text-rose-500 border-rose-100'" class="px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest border shadow-sm">
        ...
        <div class="py-6 space-y-6">
          <div class="bg-gray-50 p-5 rounded-2xl border border-gray-100">
             <p class="text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">Description & Notes</p>
             <p class="text-sm font-medium text-gray-700 leading-relaxed">{{ selectedProduct.description }}</p>
          </div>

          <div class="pt-4 border-t border-gray-100 flex flex-col gap-3">
            <h4 class="text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">Item Actions</h4>
            
            <button 
              @click="handleToggleAvailability(selectedProduct)" 
              class="w-full px-5 py-4 bg-white text-gray-700 rounded-xl text-[11px] font-bold uppercase tracking-widest border border-gray-200 shadow-sm hover:border-gray-300 active:scale-[0.98] transition-all flex items-center justify-between"
            >
    ...
    <SideDrawer :isOpen="showAddDrawer" @close="showAddDrawer = false">
        <div class="space-y-6">
          ...
          <form @submit.prevent="handleAddProduct" class="space-y-5">
            ...
            <div class="space-y-1.5">
              <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Description & Ingredients</label>
            ...
            <div class="grid grid-cols-2 gap-4">
              <div class="space-y-1.5">
                <label class="text-[10px] font-bold text-gray-400 uppercase tracking-widest ml-1">Price (₦)</label>
                <div class="relative">
              ...
              <div class="space-y-1.5">
                ...
                <input v-model="newProduct.category" placeholder="snacks, mains, etc." class="w-full px-4 py-3 bg-gray-50 border border-gray-100 rounded-xl text-sm font-bold focus:ring-2 focus:ring-parentPrimary/20 focus:bg-white focus:border-parentPrimary/30 outline-none transition-all shadow-inner uppercase tracking-wider" required />
              </div>
            </div>

            <div class="pt-6 border-t border-gray-100 mt-6">
              <button type="submit" class="w-full py-4 bg-parentPrimary text-white rounded-xl text-xs font-bold uppercase tracking-widest shadow-lg shadow-parentPrimary/20 hover:brightness-110 active:scale-[0.98] transition-all">Publish Item</button>
            </div>
          </form>
        </div>
    </SideDrawer>
