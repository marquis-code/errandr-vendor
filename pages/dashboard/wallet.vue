<template>
 ...
 class="px-8 py-4 bg-gray-900 text-white rounded-2xl font-black text-[10px] tracking-widest shadow-xl shadow-black/10 hover:bg-black active:scale-95 transition-all flex items-center gap-2"
 >
 ...
 <!-- Main Wallet Card -->
 <div class="lg:col-span-8 bg-white p-10 rounded-[3rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] relative overflow-hidden group">
 ...
 <span class="text-[9px] font-black tracking-widest">Available for Withdrawal</span>
 </div>
 ...
 <div class="space-y-8">
 <div class="space-y-1">
 <p class="text-[9px] font-black text-gray-400 tracking-widest">Total Earned</p>
 <p class="text-2xl font-black text-gray-900">₦{{ wallet?.totalEarned?.toLocaleString() || '0' }}</p>
 </div>
 <div class="space-y-1">
 <p class="text-[9px] font-black text-gray-400 tracking-widest">Monthly Growth</p>
 ...
 <p class="text-[9px] font-black text-white/40 tracking-widest">Active Settlement</p>
 ...
 <p class="text-sm font-bold text-white tracking-tight">{{ wallet.bankDetails.bankName }}</p>
 <p class="text-xl font-mono text-white/60 tracking-[0.2em]">{{ maskAccountNumber(wallet.bankDetails.accountNumber) }}</p>
 <p class="text-[10px] font-bold text-emerald-400 tracking-widest pt-2 flex items-center gap-2">
 ...
 <div class="flex justify-between items-center text-[10px] font-black tracking-widest text-white/40">
 <span>Cycle</span>
 ...
 <p class="text-[10px] font-bold text-gray-400 tracking-widest">Deep dive into your recent credits and debits.</p>
 </div>
 <div class="flex items-center gap-4">
 <div class="flex bg-gray-50 rounded-xl p-1 shadow-inner">
 <button class="px-4 py-1.5 rounded-lg text-[9px] font-black tracking-widest bg-white text-gray-900 shadow-sm">All</button>
 <button class="px-4 py-1.5 rounded-lg text-[9px] font-black tracking-widest text-gray-400 hover:text-gray-600 transition-colors">Credits</button>
 <button class="px-4 py-1.5 rounded-lg text-[9px] font-black tracking-widest text-gray-400 hover:text-gray-600 transition-colors">Payouts</button>
 </div>
 </div>
 </div>

 <div class="bg-white rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.04)] overflow-hidden">
 <UiTable
 :columns="[
 ...
 <p v-if="(item as any).order" class="text-[9px] text-[#065fdb] font-black tracking-widest">
 ...
 <template #type="{ item }">
 <span 
 class="px-3 py-1 rounded-lg text-[10px] font-black tracking-widest"
 ...
 <p class="text-xs font-bold text-gray-900 ">{{ formatDate((item as any).createdAt) }}</p>
 <p class="text-[9px] text-gray-400 font-bold mt-0.5">{{ formatTime((item as any).createdAt) }}</p>
 </div>
 </template>
 </UiTable>
 </div>
 ...
 <div class="p-8 bg-white rounded-[2.5rem] shadow-[0_8px_30px_rgb(0,0,0,0.02)] text-center space-y-6 relative overflow-hidden group border border-gray-50/50">
 <div class="absolute inset-0 bg-blue-50/20 opacity-0 group-hover:opacity-100 transition-opacity"></div>
 <div class="w-20 h-20 bg-white rounded-3xl flex items-center justify-center mx-auto shadow-[0_8px_30px_rgb(0,0,0,0.04)] text-[#065fdb] relative z-10">
 <Banknote class="w-10 h-10" />
 </div>
 <div class="relative z-10">
 <p class="text-[10px] font-black text-gray-400 tracking-widest mb-1">Current Balance</p>
 ...
 <button @click="showWithdrawModal = false" class="flex-1 py-4 bg-white border border-gray-100 text-gray-400 text-[10px] font-black tracking-widest rounded-2xl">Cancel</button>
 <button 
