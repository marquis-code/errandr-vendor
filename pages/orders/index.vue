        ...
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-50/50">
            <tr v-for="order in filteredOrders" :key="order._id" class="hover:bg-gray-50/80 transition-colors group cursor-pointer" @click="selectedOrder = order">
              ...
              <td class="py-4 px-2">
                <span :class="getStatusBadge(order.status)" class="text-[10px] font-bold uppercase tracking-widest px-2.5 py-1 rounded-md border block w-fit">
    ...
    <SideDrawer :isOpen="!!selectedOrder" @close="selectedOrder = null">
      <template v-if="selectedOrder">
        <div class="flex flex-col items-center justify-center py-6 border-b border-gray-100">
           ...
           <span :class="getStatusBadge(selectedOrder.status)" class="text-[10px] font-bold uppercase tracking-widest px-3 py-1.5 rounded-full border shadow-sm">
        ...
        <div class="py-6 space-y-6">
          <div class="bg-gray-50/80 rounded-2xl p-6 space-y-4 shadow-inner border border-gray-100/50">
             <div class="flex justify-between items-center mb-2">
               <p class="text-[10px] font-bold text-gray-400 uppercase tracking-widest">Order Contents</p>
          ...
          <div class="pt-4 border-t border-gray-100 flex flex-col gap-3">
            <h4 class="text-[10px] uppercase font-bold tracking-widest text-gray-400 mb-2">Vendor Actions</h4>
            
            <button 
              v-if="selectedOrder.status === 'confirmed'" 
              @click="updateStatus(selectedOrder._id, 'preparing'); selectedOrder = null;" 
              class="w-full px-5 py-4 bg-parentPrimary text-white rounded-xl text-[11px] font-bold uppercase tracking-widest shadow-lg shadow-parentPrimary/20 hover:brightness-110 active:scale-[0.98] transition-all flex items-center justify-center gap-2"
            >
            ...
            <button 
              ...
              class="w-full px-5 py-4 bg-gray-900 text-white rounded-xl text-[11px] font-bold uppercase tracking-widest shadow-lg shadow-gray-900/10 hover:bg-black active:scale-[0.98] transition-all flex items-center justify-center gap-2"
            >
            ...
            <NuxtLink 
               ...
               class="w-full px-5 py-3.5 bg-white text-gray-500 rounded-xl text-[11px] font-bold uppercase tracking-widest border border-gray-200 hover:text-gray-900 hover:border-gray-300 transition-all text-center mt-2 group"
            >
